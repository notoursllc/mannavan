...
export default {
    ...
    methods: {
        async fetchTypes() {
            try {
                this.types = await this.matmix_list();
            }
            catch(e) {
                this.$errorMessage(
                    e.message,
        ...
        async onTypeDelete(data) {
            try {
                ...
                try {
                    const typeJson = await this.matmix_delete(data.id);

                    if(!typeJson) {
                        throw new Error(this.$t('Type not found'));
                    }
        ...
        async onUpsertClick(data) {
            try {
                if(isObject(data) && data.id) {
                    const type = await this.matmix_get(data.id);
                    ...
                    const types = await this.matmix_list();
                    this.dialog.type = {
                        is_available: true,
        ...
        async onUpsertFormSave(type) {
            try {
                const p = await this.matmix_upsert(type);

                if(!p) {
                    throw new Error(this.$t('Error updating type'));
                }

                let title = type.id ? 'Type updated successfully' : 'Type added successfully';
...
<template>
    <div>
        ...
        <app-dialog
            :title="dialog.type.id ? 'Edit type' : 'Add type'"
