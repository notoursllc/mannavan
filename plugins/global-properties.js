...
export default async (ctx) => {
    ...
    const [ productTypes, productSubTypes, productSkuAccentMessages ] = await Promise.all([
        ...
        ctx.app.$api.productSkuAccentMessages.all()
    ]);

    ctx.store.dispatch('product/PRODUCT_TYPES', productTypes);
    ctx.store.dispatch('product/PRODUCT_SUBTYPES', productSubTypes);
    ctx.store.dispatch('product/PRODUCT_SKU_ACCENT_MESSAGES', productSkuAccentMessages);
